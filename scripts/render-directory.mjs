...
function page(shell, {title, description, path, main, schema, origin, canonicalOrigin, revision, directory = false}) {
  const url = origin + path;
  // Canonical points to the primary orkes.io page (trailing-slash-free) so the
  // duplicate OSS copy defers to it and doesn't compete in search.
  const canonical = canonicalOrigin + (path === '/' ? '' : path.replace(/\/$/, ''));
  const head = `<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escape(title)}</title><meta name="description" content="${escape(description)}"><meta name="robots" content="index,follow,max-image-preview:large"><link rel="canonical" href="${escape(canonical)}"><meta property="og:type" content="website"><meta property="og:site_name" content="Conductor"><meta property="og:title" content="${escape(title)}"><meta property="og:description" content="${escape(description)}"><meta property="og:url" content="${escape(url)}"><meta name="twitter:card" content="summary"><meta name="twitter:title" content="${escape(title)}"><meta name="twitter:description" content="${escape(description)}"><link rel="icon" href="/assets/favicon.svg" type="image/svg+xml"><link rel="preload" href="/assets/fonts/inter-latin.woff2" as="font" type="font/woff2" crossorigin><link rel="stylesheet" href="/assets/site.css?v=${revision}"><link rel="stylesheet" href="/use-cases/directory.css?v=${revision}"><script type="application/ld+json">${json(schema)}</script>`;
